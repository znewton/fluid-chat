...
import { canWrite, generateLargeMessage, generateLoremIpsumMessage, getRandomUser } from "../utils";
...
export const GenTrafficButton: React.FunctionComponent<IGenTrafficButtonProps> = (props) => {
    ...
    const handleToggleGenTraffic: React.MouseEventHandler<HTMLButtonElement> = (e) => {
        setWorking(!working);
    };
    ...
    return (
        <button type="button" onClick={handleToggleGenTraffic} disabled={disableInputs}>
            <FontAwesomeIcon icon={["fas", working ? "circle-stop" : "circle-play"]} title="toggle traffic generation" />
            &nbsp;&nbsp;Gen Traffic
        </button>
    );
}